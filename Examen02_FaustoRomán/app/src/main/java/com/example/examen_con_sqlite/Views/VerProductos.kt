...
class VerProductos : AppCompatActivity() {
    ...
    var listaDeProductos: List<Producto> = emptyList()
    lateinit var adaptadorProductos: ProductoAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!empresaId.isNullOrEmpty()) {
            EmpresaFirestore().obtenerUnEmpresa(empresaId).addOnSuccessListener { documentSnapshot ->
                    if (documentSnapshot.exists()) {
                        empresa = EmpresaFirestore.crearEmpresaFromDocument(documentSnapshot)
                        ...
                        adaptadorProductos = ProductoAdapter(this, listaDeProductos)
                        ...
                        val nombreProductoAgregada =
                            intent.getStringExtra("NOMBRE_PRODUCTO_AGREGADA")
                        ...
                        if (!nombreProductoAgregada.isNullOrEmpty()) {
                            ...
                            mostrarSnackbar("Canción Agregada: $nombreProductoAgregada")
                        }

                        // Después de agregar la canción a Firestore
                        EmpresaFirestore().obtenerProductosPorEmpresaId(empresa.id)
                            .addOnSuccessListener { querySnapshot ->
                                // Mapear los documentos a objetos Cancion
                                listaDeProductos = querySnapshot.documents.map {
                                    ProductoFirestore.createProductoFromDocument(it)
                                }
                                ...
                                val listViewProductos =
                                    findViewById<ListView>(R.id.lv_Listado_Productos)
                                listViewProductos.adapter = adaptadorProductos
                        ...
                        val listViewProductosContextMenu =
                            findViewById<ListView>(R.id.lv_Listado_Productos)
                        registerForContextMenu(listViewProductosContextMenu)
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            ...
            R.id.mi_EliminarProducto -> {
                ...
                ProductoFirestore().removeProducto(idProductoSeleccionada)

                // Actualizar la lista de canciones
                val task = ProductoFirestore().obtenerProductosPorEmpresaId(empresa.id)
    ...
    fun irActividadConID(clase: Class<*>, empresaId: String?) {
        val intent = Intent(this, clase)
    ...
    fun irActividadConIDs(clase: Class<*>, idEmpresa: String?, idProducto: String?) {
        val intent = Intent(this, clase)
